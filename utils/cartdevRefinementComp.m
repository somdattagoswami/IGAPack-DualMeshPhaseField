function[Basis]=cartdevRefinementComp(PHTelem,controlPts,geometry,Basis,markRef,cornerElem)
    ...
    for indexRef = 1:length(markRef{indexPatch})
        if markRef{indexPatch}(indexRef)
            if isempty(PHTelem{indexPatch}(indexRef).children)
                ...
                for iRow = 1:size(cornerElem,1)
                    if (cornerElem(iRow,1)==indexPatch) && (cornerElem(iRow,2)==indexRef)
                        Basis.isComp{indexPatch}{indexRef}(1:ngaussX*ngaussY) = 1;
                    else
                        Basis.isComp{indexPatch}{indexRef}(1:ngaussX*ngaussY) = 0;
                    end
                end
            else
                ...
                for ichild = 1:4
                    ...
                    for iRow = 1:size(cornerElem,1)
                        if (cornerElem(iRow,1)==indexPatch) && (cornerElem(iRow,2)==i)
                            Basis.isComp{indexPatch}{i}(1:ngaussX*ngaussY) = 1;
                        else
                            Basis.isComp{indexPatch}{i}(1:ngaussX*ngaussY) = 0;
                        end
                    end
                end
                ...
                Basis.isComp{indexPatch}{indexRef} = [];
            end
        end
    end
end
