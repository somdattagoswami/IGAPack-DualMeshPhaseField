function [PHTelem,dimBasis,coordinates] = crossInsert3D(PHTelem,controlPts,elm_index,dimBasis,geometry)
...
reg = getRegionIndices3D(geometry.p,geometry.q,geometry.r);% Define corner indices

for indexElem=1:length(elm_index)
    e = elm_index(indexElem);
    ...
    %add children elements and their sibling neighbors
    ...
    % Whether T-junctions are being removed
    ...
    for ijunct = RTjunct
        ...
        % Store the parent neighbors in a list to handle possible multiple parents.
        parent_neighbor = zeros(1,3);
        
        % Update the neighbors of the neighbor with children elements so we
        % Get correct knotUln, knotUrn, knotVdn, knotVun, knotWbn, knotWtn
        switch ijunct
            case 1
        ...
        %update the value of dimBasisTemp
    ...
    if (indexElem/100)==round(indexElem/100)
        disp(['Refined ', num2str(indexElem), ' elements out of ', num2str(length(elm_index))])
    end
end
end