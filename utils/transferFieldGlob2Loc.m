function [fieldDataPatch] = transferFieldGlob2Loc(PHTelem,PhmeshInfo,fieldData)
% Transfer the fieldData inidices from the global array to the local patch structure

numPatches = length(PHTelem);
fieldDataPatch = cell(1, numPatches);

for iPatch = 1:numPatches
    fieldDataPatch{iPatch} = zeros(PhmeshInfo.dimBasis(iPatch),size(fieldData,2));
