function[Basis]=cartdevComp(PHTelem,controlPts,geometry,cornerElem)
...
Basis.isComp = cell(1,numPatches);

for indexPatch = 1:length(PHTelem)
    
    Basis.isComp(indexPatch) = {cell(length(PHTelem{indexPatch}),1)};
    Basis.isComp{indexPatch}(:) = {zeros(1,ngaussX*ngaussY)};
