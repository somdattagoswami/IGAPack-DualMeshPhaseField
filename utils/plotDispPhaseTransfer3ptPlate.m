function plotDispPhaseTransfer3ptPlate(PhPHTelem,EPHTelem,PhcontrolPts,EcontrolPts,Phidisp,Edisp,PhmeshInfo,geometry,Mater)
...
for indexPatch = 1:length(PhPHTelem)
    for i=1:length(PhPHTelem{indexPatch})
        if isempty(PhPHTelem{indexPatch}(i).children)
            ...
            indexParent = i;
            while PhPHTelem{indexPatch}(indexParent).level > 0
            ...
            if isempty(EPHTelem{indexPatch}(indexParent).children)
                ...
                for ilevel = 1 : geometry.maxERefLevel
                    branchCount = 0;
            ...
            for ii=1:nx
                for jj=1:ny
                    ...
                    Esctrx = EPHTelem{indexPatch}(EelmtNum).nodesGlobal(1:nument);
                    Edsctrx = reshape([2*Esctrx-1;2*Esctrx],1,dim*nument);
