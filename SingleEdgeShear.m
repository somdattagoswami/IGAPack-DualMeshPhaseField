...
[geometry,Mater,Fract,Integ] = Input_shearPlate;
[EPHTelem,PhPHTelem,EcontrolPts,PhcontrolPts,EmeshInfo,PhmeshInfo] = modelDualMesh(geometry,'singleEdgeShear');
EBasis = cartdev(EPHTelem,EcontrolPts,geometry);
PhBasis = cartdev(PhPHTelem,PhcontrolPts,geometry);
Edirichlet = initialBC_shear(EPHTelem,geometry);
...
plot3 = subplot(2,2,3);
plot(0,0,'Marker','*','MarkerSize',3,'LineWidth',1)
xlim([0 0.012])
ylim([0 500])
...
for istep = 1:Integ.nstep
    ...
    compTreacShear(stiffUU,Edisp,Integ.tfacto,Edirichlet,file_name);
    tic
    if(mod(istep,Integ.nprint) == 0)% Print Results
        
        fprintf('Done step: %5d\n',istep);
        plotDispPhaseTransfer2D_shear(PhPHTelem,EPHTelem,PhcontrolPts,EcontrolPts,Phidisp,Edisp,PhmeshInfo,geometry,Mater);
        plot1 = subplot(2,2,1);
